...
Amazon sales rank data was obtained from https://www.kaggle.com/ucffool/amazon-sales-rank-data-for-print-and-kindle-books#ranks.zip.  Python was used to create a csv of the data.  Python code can be found here:  https://github.com/jitsen-design/Data-607-Final-Project/blob/master/amazon-sales-rank-data-for-print-and-kindle-books/code/amazonsalesranks.ipynb  

Load the NY Times bestseller list data from the SQL database table to a data frame.  
Load the Amazon sales rank data from the Google Doc csv to a data frame, send it to a table in the SQL database, and then reload.  
...
#load the Amazon sales rank data from the Google Doc
#id <- "1GT0nqTeXQMv8ujJO2KBW00dIfOVMcJSk" # google file ID
#sales_ranks <- read.csv(sprintf("https://docs.google.com/uc?id=%s&export=download", id))
...
* Books on advice, expeditions, and culture have positive reviewn sentiment and books on celebrities, fiction, and business have lower review  sentiment.  
